...
contract CosmodinosOmega is Ownable, ERC721A, ReentrancyGuard {
    ...
    function setMerkleRoot(uint256 _allowlistPrice, bytes32 _merkleRoot)
        public
        onlyOwner
    {
    ...
    function setAllowlistMintIsOpen(bool allowlistMintIsOpen_)
        public
        onlyOwner
    {
    ...
    function devMint(uint256 count) external onlyOwner {
        ...
        require(
            _numberMinted(msg.sender).add(count) <= amountForDevs,
            "too many dev mint"
        );
        _safeMint(msg.sender, count);
    }
